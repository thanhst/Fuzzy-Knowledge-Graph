...
rem ==============================
rem ICTA full flow runner (one-click)
rem ==============================
...
rem --- Dependency precheck in .venv only
...
echo [INFO] ============================================
echo [INFO] ICTA flow started: %date% %time%
...
echo [STEP] 2/6 Process image...
"%PYTHON_EXE%" "%SOURCE_DIR%\Src\Flow\process_image.py"
if errorlevel 1 goto :fail
...
echo [STEP] 4/6 Select image features...
"%PYTHON_EXE%" "%SOURCE_DIR%\Src\Flow\select_image_features.py" --k %K_IMG%
if errorlevel 1 goto :fail

echo [STEP] 5/6 Fusion...
"%PYTHON_EXE%" "%SOURCE_DIR%\Src\Flow\fusion.py"
if errorlevel 1 goto :fail

echo [STEP] 6/6 Run FKG flow...
...
echo [INFO] ============================================
echo [INFO] ICTA flow completed successfully.
echo [INFO] Result folder: %SOURCE_DIR%\Data\result\ICTA
...
:fail
echo [ERROR] ICTA flow failed. Please check the error log above.
