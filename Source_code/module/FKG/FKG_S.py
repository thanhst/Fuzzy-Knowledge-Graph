...
class FKGS:
    ...
    def FKGS(self,df,testdf,Turn = None,Modality = None,ran = None, e = None):
        ...
        for i in range(1,6):
            ...
            print("---Finish---\n")
        # print(":"*100)
        # print('sampling_time:', round(sum(sampling_time) / len(sampling_time), 2) if sampling_time else 0)
        # print('train_time:', round(sum(train_time) / len(train_time), 2) if train_time else 0)
        # print('test_time:', round(sum(test_time) / len(test_time), 2) if test_time else 0)
        # print('accuracy:', round(sum(self.listAcc) / len(self.listAcc), 2) if self.listAcc else 0)
        # print('precision:', round(sum(self.listPre) / len(self.listPre), 2) if self.listPre else 0)
        # print('recall:', round(sum(self.listRe) / len(self.listRe), 2) if self.listRe else 0)
        # print('Total time:', round(sum(total_time) / len(total_time), 2) if total_time else 0)
        # print(":" * 100)

        # Mean ± Std
        print(f'sampling_time: {np.mean(sampling_time):.2f} ± {np.std(sampling_time):.2f}')
        print(f'train_time: {np.mean(train_time):.2f} ± {np.std(train_time):.2f}')
        print(f'test_time: {np.mean(test_time):.2f} ± {np.std(test_time):.2f}')
        print(f'accuracy: {np.mean(self.listAcc):.2f} ± {np.std(self.listAcc):.2f}')
        ...
        print(f'train_time variance: {np.var(train_time):.2f}')
        print(f'test_time variance: {np.var(test_time):.2f}')
        print(f'accuracy variance: {np.var(self.listAcc):.2f}')
