...
def process_image(folder_save,file_path,k_img=7):
    ...
    def preprocessing_image_feature_selection():
        ...
        index = df.iloc[:, 0]  # Lấy cột đầu tiên
        y = df.iloc[:, -1].values
        Fimg = df.iloc[:, 1:-1].values
        pd.DataFrame(Fimg).to_csv(os.path.join(base_path,f'data/Dataset_diabetic/{folder_save}/image_fts_norm.csv'),index=False)
        from sklearn.feature_selection import SelectKBest, mutual_info_classif
        selected_Fimg = pd.DataFrame(Fimg)
        selected_Fimg.insert(0, 'image_id', index.values)
        selected_Fimg = pd.concat([selected_Fimg,pd.DataFrame(y)],axis=1)
        pd.DataFrame(selected_Fimg).to_csv(os.path.join(base_path,f'data/Dataset_diabetic/{folder_save}/image_fts_norm.csv'),index=False)
    # preprocessing_image()
    preprocessing_image_feature_selection()
    
def process_table(file_path,folder_save,k_tab = 10):
    ...
    X = dfData_scaled.iloc[:,:-1].values
    y = dfData_scaled.iloc[:,-1].astype(int).values
    selector_tab = SelectKBest(score_func=mutual_info_classif, k=k_tab)
    selected_Ftab = selector_tab.fit_transform(X, y)

    df_Ftab = pd.DataFrame(selected_Ftab)
    df_Ftab = pd.concat([df_Ftab, pd.DataFrame(y)], axis=1)
...
def processing(file_path_img,file_path_table, folder_save,k_tab,k_img):
    os.makedirs(os.path.join(base_path,f'data/Dataset_diabetic/{folder_save}'), exist_ok=True)
    process_table(file_path=file_path_table,folder_save=folder_save,k_tab=k_tab)
    process_image(file_path=file_path_img,folder_save=folder_save,k_img=k_img)
    ...
    Fimg_select = pd.DataFrame(feature_selection.select_features(X=Fimg,y = Label, k = k_img))
    Ftab_select = pd.DataFrame(feature_selection.select_features(X=Ftab,y = Label, k = k_tab))
    Ffused_df = np.concatenate([Fimg_select, Ftab_select], axis=1)
    Ffused_df = pd.concat([pd.DataFrame(Ffused_df), pd.DataFrame(Label)], axis=1)
    Ffused_df = pd.DataFrame(Ffused_df)
