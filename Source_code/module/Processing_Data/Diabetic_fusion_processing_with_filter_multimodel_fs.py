...
def processing(file_path_img,file_path_table, folder_save,k_img=10,k_tab=10,corr=0.9):
    os.makedirs(os.path.join(base_path,f'data/Dataset_diabetic/{folder_save}'), exist_ok=True)
    ...
    Ffused_df = pd.DataFrame(filter_multimodal_selection.filter_multimodal_selection(Fimg=Fimg,Ftab=Ftab,target=Label.values.ravel(),k_img=k_img,k_tab=k_tab,corr_threshold=corr))
