...
Diabetic_image_processing.processing(file_path_table="data/Dataset_diabetic/data_process.csv",file_path_img="data/Image/fundus_photo",folder_save="Image_feature")
...
for i in r:
    for j in e:
        ...
        test = [[int(float(x)) for x in row] for row in testdf.values]
        fkg_instance = FKGS()
        fkg_instance.FKGS(df = base,testdf=test,Turn=None,Modality="Diabetic Retinopathy Image Feature",ran=i,e=j,folderPath=project_root)
        print("-"*100)
